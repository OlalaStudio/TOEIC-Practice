...
-(void)didFinishPlayer{
    ...
    NSArray *anwsers = [_dataDic objectForKey:@"question"];
    NSInteger correctCount = 0;
    
    if([self checkAnwser:_anwser1 validAnwser:[anwsers objectAtIndex:0]]){
        correctCount ++;
    }
    
    if([self checkAnwser:_anwser2 validAnwser:[anwsers objectAtIndex:1]]){
        correctCount ++;
    }
    
    if([self checkAnwser:_anwser3 validAnwser:[anwsers objectAtIndex:2]]){
        correctCount ++;
    }
    ...
    if (correctCount > 0) {
        NSString *correctcountStr = [NSString stringWithFormat:@"%ld / %lu",(long)correctCount,(unsigned long)[anwsers count]];
